...
{
    fmt::print("${:04X}: {}\n", cpudbg.getreg(CPUDebugger::Reg::PC), cpudbg.curr_instr_str());
}

void CliDebugger::print_cpu_status() const
{
    using enum CPUDebugger::Reg;
    fmt::print("PC: ${:02X} A: ${:02X} X: ${:02X} Y: ${:02X} S: ${:02X}\n"
               "Flags: [{}]\n"
               "Cycles: {}\n",
               cpudbg.getreg(PC), cpudbg.getreg(Acc), cpudbg.getreg(X),
               cpudbg.getreg(Y),  cpudbg.getreg(SP),  cpudbg.curr_flags_str(),
               cpudbg.cycles());
}

void CliDebugger::print_ppu_status() const
{
    using enum PPUDebugger::Reg;
    auto onoff = [](auto val) { return val ? "ON" : "OFF"; };
    u8 ctrl = ppudbg.getreg(Ctrl);
    u8 mask = ppudbg.getreg(Mask);
    u8 status = ppudbg.getreg(Status);
    ...
    fmt::print("PPUCTRL ($2000): {:08b}:\n"
               ...
               ctrl, ppudbg.nt_base_addr(),    (ctrl & 4) == 0 ? 1 : 32,     util::getbit(ctrl, 4) * 0x1000,
