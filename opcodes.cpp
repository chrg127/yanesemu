...
void CPU::instr_jmp_ind()
{
    ...
    if (op.low == 0xFF) {
        ...
        pc.high = read(op.reg & 0xFF00);
    } else {
        pc.low = read(op.reg);
        pc.high = read(op.reg+1);
    }
    last_cycle();
}
